...
public class ModerateAdvertComponent : ComponentBase
{
    ...
    protected async Task Grid0LoadData(LoadDataArgs args)
    {
        try
        {
            if (!_isSorted)
            {
                ...
                getAdvertsResult = postGetAdvertsResult.Adverts.Where(_ => _.ReviewStatus == Common.Enums.ReviewStatus.NotReviewed).AsODataEnumerable();
    ...
    protected async Task Denie(MouseEventArgs args, dynamic data)
    {
        try
        {
            if (await DialogService.Confirm("Вы уверены, что хотите отказать в публикации объявления?", "Отказ") == true)
            {
                await _advertsClient.SetManagedStatusAdvert(Guid.Parse($"{data.Id}"), false);
                await grid0.Reload();
    ...
    protected async Task Review(MouseEventArgs args, dynamic data)
    {
        try
        {
            if (await DialogService.Confirm("Вы уверены, что хотите разместить объявление?", "Размещение") == true)
            {
                await _advertsClient.SetManagedStatusAdvert(Guid.Parse($"{data.Id}"), true);
                await grid0.Reload();
            }
        }
        catch
