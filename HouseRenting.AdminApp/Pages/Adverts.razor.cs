using HouseRenting.Common.Dto.Advert;
using HouseRenting.Common.RefitContracts;
using Microsoft.AspNetCore.Components;
using Microsoft.AspNetCore.Components.Web;
using Microsoft.JSInterop;
using Radzen.Blazor;
using Radzen;
using System.Reflection;

namespace HouseRenting.AdminApp.Pages;

public class AdvertsComponent : ComponentBase
{
    private const string NoImage = "";
    private bool _isSorted = false;
    private Dictionary<Guid, int> PhotoesIndexes = new();
    
    [Parameter(CaptureUnmatchedValues = true)]
    public IReadOnlyDictionary<string, dynamic> Attributes { get; set; } = null!;

    public void Reload()
    {
        InvokeAsync(StateHasChanged);
    }

    [Inject]
    private IAdvertsClient _advertsClient { get; set; } = null!;

    [Inject]
    private IJSRuntime JSRuntime { get; set; } = null!;

    [Inject]
    private NavigationManager UriHelper { get; set; } = null!;

    [Inject]
    private DialogService DialogService { get; set; } = null!;

    [Inject]
    private TooltipService TooltipService { get; set; } = null!;

    [Inject]
    private ContextMenuService ContextMenuService { get; set; } = null!;

    [Inject]
    private NotificationService NotificationService { get; set; } = null!;

    protected RadzenDataGrid<GetAdvertResponseDto> grid0 = null!;

    protected IEnumerable<GetAdvertResponseDto> _getAdvertsResult = null!;
    protected IEnumerable<GetAdvertResponseDto> getAdvertsResult
    {
        get
        {
            return _getAdvertsResult;
        }
        set
        {
            if (!object.Equals(_getAdvertsResult, value))
            {
                var args = new PropertyChangedEventArgs() { Name = "getAdvertsResult", NewValue = value, OldValue = _getAdvertsResult };
                _getAdvertsResult = value;
                Reload();
            }
        }
    }

    int _getAdvertsCount;
    protected int getAdvertsCount
    {
        get
        {
            return _getAdvertsCount;
        }
        set
        {
            if (!object.Equals(_getAdvertsCount, value))
            {
                var args = new PropertyChangedEventArgs() { Name = "getAdvertsCount", NewValue = value, OldValue = _getAdvertsCount };
                _getAdvertsCount = value;
                Reload();
            }
        }
    }

    protected async Task Button0Click(MouseEventArgs args)
    {
        var dialogResult = await DialogService.OpenAsync<AddAdvert>("Добавить объявление", null);
        await grid0.Reload();

        await InvokeAsync(() => { StateHasChanged(); });
    }

    protected async Task Grid0LoadData(LoadDataArgs args)
    {
        try
        {
            if (!_isSorted)
            {
                var postGetAdvertsResult = await _advertsClient.GetAdverts(new());
                getAdvertsResult = postGetAdvertsResult.Adverts.Where(_ => _.ReviewStatus == Common.Enums.ReviewStatus.Reviewed).AsODataEnumerable();

                getAdvertsCount = postGetAdvertsResult.Adverts.Count;
            }
            _isSorted = false;
        }
        catch
        {
            NotificationService.Notify(new NotificationMessage() { Severity = NotificationSeverity.Error, Summary = $"Error", Detail = $"Unable to load Adverts" });
        }
    }

    protected async Task Grid0RowSelect(GetAdvertResponseDto args)
    {
        var dialogResult = await DialogService.OpenAsync<EditAdvert>("Изменить объявление", new Dictionary<string, object>() { { "Id", args.Id }, { "CanEdit", !args.IsActual } });
        await grid0.Reload();

        await InvokeAsync(() => { StateHasChanged(); });
    }

    protected async Task GridDeleteButtonClick(MouseEventArgs args, dynamic data)
    {
        try
        {
            if (!data.IsActual)
            {
                NotificationService.Notify(new NotificationMessage() { Severity = NotificationSeverity.Error, Summary = $"Ошибка", Detail = $"Невозможно удалить объявление! Квартира сдана!" });
            }
            if (await DialogService.Confirm("Вы уверены, что хотите удалить объявление?", "Удаление") == true)
            {
                await _advertsClient.DeleteAdvert(Guid.Parse($"{data.Id}"));
                await grid0.Reload();
            }
        }
        catch
        {
            NotificationService.Notify(new NotificationMessage() { Severity = NotificationSeverity.Error, Summary = $"Error", Detail = $"Невозможно удалить объявление!" });
        }
    }

    public string GetImage(GetAdvertResponseDto advert)
    {
        if (!PhotoesIndexes.ContainsKey(advert.Id))
        {
            PhotoesIndexes.Add(advert.Id, 0);
        }

        var image = advert.MediaFiles.Skip(PhotoesIndexes[advert.Id]).Take(1).FirstOrDefault();

        if (!string.IsNullOrWhiteSpace(image))
        {
            var fs = string.Format("data:image/jpg;base64,{0}", image);
            return fs;
        }
        else
        {
            return string.Format("data:image/jpg;base64,{0}", NoImage);
        }
    }

    public void PrevImg(GetAdvertResponseDto advert)
    {
        var imagesCount = advert.MediaFiles.Count;
        var currentIndex = PhotoesIndexes[advert.Id];
        var index = currentIndex - 1;

        if(currentIndex == 0)
        {
            index = imagesCount - 1;
        }

        PhotoesIndexes[advert.Id] = index;
    }

    public void NextImg(GetAdvertResponseDto advert)
    {

        var imagesCount = advert.MediaFiles.Count;
        var currentIndex = PhotoesIndexes[advert.Id];
        var index = currentIndex + 1;

        if (currentIndex == imagesCount - 1)
        {
            index = 0;
        }

        PhotoesIndexes[advert.Id] = index;
    }

    public void OnSort(DataGridColumnSortEventArgs<GetAdvertResponseDto> args)
    {
        var sortOrder = args.SortOrder;
        var sortProperty = args.Column.GetSortProperty();
        PropertyInfo prop = typeof(GetAdvertResponseDto).GetProperty(sortProperty)!;

        if (sortOrder == SortOrder.Descending)
        {
            _getAdvertsResult = _getAdvertsResult.OrderByDescending(x => prop.GetValue(x, null)).ToList();
        }
        if(sortOrder == SortOrder.Ascending)
        {
            _getAdvertsResult = _getAdvertsResult.OrderBy(x => prop.GetValue(x, null)).ToList();
        }
        _isSorted = true;
    }
}
